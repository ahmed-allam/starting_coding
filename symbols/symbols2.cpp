...
int main(int argc, char *argv[])
{
	...
	ofstream myfile2;
	myfile2.open("symbols_array6.txt");
		...
		while (myfile.good())
		{
			...
			else if ((line.find_first_of(' ', 0) == 0) || (line.find_first_of('\t', 0) == 0))
			{
				...
				else if ((x > 0) && (line.substr(x, 1) == "\""))
				{
					...
					//newline = "(symbols_enum::"+symbol + ","+"\""+ symbol +"\")";// + symbol_value + "," + comment_value + posix_str;
					newline =
						"else if (s_capitalized == \"" + symbol + "\"" + ") {" +"\n"+
						"	v = boost::any(symbols_enum::" + symbol + ");" +"\n"+
						"}";
			...
			else
			{
				...
				//newline = "{\"" + symbol + "\", " + symbol_value + "}," + comment_value + posix_str;
				//newline = symbol + ",";// +symbol_value + "," + comment_value + posix_str;
				//newline = "(symbols_enum::" + symbol + "," + "\"" + symbol + "\")";
				//newline = "(symbols_enum::" + symbol + "," + "\"" + symbol + "\")";
				newline =
					"else if (s_capitalized == \"" + symbol + "\"" + ") {" + "\n" +
					"	v = boost::any(symbols_enum::" + symbol + ");" + "\n" +
